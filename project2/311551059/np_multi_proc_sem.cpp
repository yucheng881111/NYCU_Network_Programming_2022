...
struct buffer {
	bool lock;		// perform busy waiting lock, abandoned.
...
void HandleOneLineCmd(vector<Command> &cmd_list, bool has_outer_pipe) {
	if (broadcast_receive_msg != "" && broadcast_pipe_msg != "") {
		...
		usleep(1000);		// to make sure that SIGUSR1 complete cout in other process
		sem_post(sem);
		//shm_buffer->lock = 0;
...
void npshell(int id) {
	...
	while(1) {
		...
		if (s == "setenv") {
			...
			sem_wait(sem);
			strcpy(shm_buffer->broadcast_or_tell, message.c_str());
			//sleep(5); // just for testing semaphore
			for (int i = 1; i < 31; ++i) {
				if (clients[i].pid != 0) {
					kill(clients[i].pid, SIGUSR1);
				}
...
int main(int argc, char *argv[], char *envp[]) {
	...
	sem_clients = sem_open("pSem1", O_CREAT, 0644, 1);
